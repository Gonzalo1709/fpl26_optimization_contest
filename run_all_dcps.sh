...
echo "dcp,status,exit_code,console_log,run_artifact_dir,optimizer_run_dir,token_usage_json,start_time,end_time" > "${SUMMARY_CSV}"
...
for dcp in "${DCP_FILES[@]}"; do
  ...
  if [[ "${MODE}" == "test" ]]; then
    cmd=("make" "-C" "${SCRIPT_DIR}" "run_test" "DCP=${dcp}" "MAX_NETS=${MAX_NETS}" "RUN_CWD=${run_artifact_dir}")
  else
    cmd=("make" "-C" "${SCRIPT_DIR}" "run_optimizer" "DCP=${dcp}" "RUN_CWD=${run_artifact_dir}")
    if [[ -n "${OPT_ARGS}" ]]; then
  ...
  printf '"%s",%s,%d,"%s","%s","%s","%s","%s","%s"\n' \
    "${dcp}" "${status}" "${exit_code}" "${log_file}" "${run_artifact_dir}" "${optimizer_run_dir}" "${token_usage_json}" "${start_time}" "${end_time}" >> "${SUMMARY_CSV}"

  echo "[${index}/${total}] ${status} - console log: ${log_file}"
  if [[ -n "${token_usage_json}" ]]; then
    echo "[${index}/${total}] Token usage JSON: ${token_usage_json}"
  fi
  echo
done
